...
wp.media.view.AttachmentsView = wp.media.view.Attachments.extend({
    initialize: function () {
        // Add state for the view type (default: grid)
        this.viewType = 'grid';
        wp.media.view.Attachments.prototype.initialize.apply(this, arguments);
    },
    ...
    render: function () {
        ...
        // Render attachments based on viewType
        if (this.viewType === 'grid') {
            this.$el.append('<ul class="attachment-grid"></ul>');
            this.collection.each(this.renderAttachment.bind(this));
    ...
    renderToolbar: function () {
        const toolbar = wp.template('toolbar-switcher'); // Create a template for the toolbar
        this.$el.prepend(toolbar());

        // Add click event listener for view switcher
        this.$('.view-switcher').on('click', this.toggleView.bind(this));
    },
    ...
    renderAttachment: function (attachment) {
        ...
        if (this.viewType === 'grid') {
            const attachmentView = new wp.media.view.Attachment({ model: attachment });
            this.$('.attachment-grid').append(attachmentView.render().el);
        } else if (this.viewType === 'table') {
            const attachmentView = new wp.media.view.Attachment({ model: attachment });
            const attachmentRow = `
                <tr>
                    <td>${attachment.get('name')}</td>
                    <td>${attachment.get('date')}</td>
                    <td>${attachment.get('size')}</td>
                </tr>
            `;
            this.$('.attachment-table tbody').append(attachmentRow);
        }
    }
});
