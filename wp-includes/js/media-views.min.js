...
Attachment = View.extend(/** @lends wp.media.view.Attachment.prototype */{
	tagName: function() {
		if (this.controller.isModeActive('listView')) {
			return 'tr';
		} else {
			return 'li';
		}
	...
	attributes: function() {
		if (this.controller.isModeActive('listView')) {
			return {
				...
				'class':          "author-self status-inherit",
	...
	initialize: function() {
		if (this.controller.isModeActive('listView')) {
			this.template = wp.template('attachment-list');
		}
...
Attachments = View.extend(/** @lends wp.media.view.Attachments.prototype */{
	...
	template: function() {
		if (this.controller.isModeActive('listView')) {
			return wp.template('list-table-body');
		} else {
			return wp.template('empty-template');
		}
	},
	attributes: function () {
	...
	// attributes: function() {
	// 	if ((wp.media.controller.State.currentView == 'list')) {
	// 		return function () {
	// 			return {style: "width: 100%;"};
	// 		}
	// 	} else {
	...
	render: function() {
		if (!(this.controller.isModeActive('listView'))) {
			...
			_.delay( _.bind( function() {
				var theadHtml = `<thead>
				<tr>
					<th scope="col" id="file" class="manage-column column-title column-primary desc" abbr="File"><a><span>File</span><span class="sorting-indicators"><span class="sorting-indicator asc" aria-hidden="true"></span><span class="sorting-indicator desc" aria-hidden="true"></span></span> <span class="screen-reader-text">Sort ascending.</span></a></th>
					<th scope="col" data-sort="title" id="title" class="manage-column column-title column-primary sortable desc" abbr="Title" aria-sort="none"><a><span>Title</span><span class="sorting-indicators"><span class="sorting-indicator asc" aria-hidden="true"></span><span class="sorting-indicator desc" aria-hidden="true"></span></span> <span class="screen-reader-text">Sort ascending.</span></a></th>
					<th scope="col" data-sort="author" id="author" class="manage-column column-author sortable desc" abbr="Author" aria-sort="none"><a ><span>Author</span><span class="sorting-indicators"><span class="sorting-indicator asc" aria-hidden="true"></span><span class="sorting-indicator desc" aria-hidden="true"></span></span> <span class="screen-reader-text">Sort ascending.</span></a></th>
					<th scope="col" data-sort="date" id="date" class="manage-column column-date sortable desc" aria-sort="descending" abbr="Date"><a ><span>Date</span><span class="sorting-indicators"><span class="sorting-indicator asc" aria-hidden="true"></span><span class="sorting-indicator desc" aria-hidden="true"></span></span></a></th>	</tr>
				</thead>`;
				if (!$('#custom-media-table').find('thead').length) {
					$('#custom-media-table').prepend(theadHtml);
				}
				var that = this;
				$('.sortable').click(function(event) {
					var sortData = $(this).data();
					var sortColumn = sortData.sort;
					that.changeSortable(sortColumn);
				})
			}, this ), 0 );
			this.prepare();
	...
	changeSortable: function(columnName) {
		var collection = this.collection;
...
AttachmentsBrowser = View.extend(/** @lends wp.media.view.AttachmentsBrowser.prototype */{
	...
	initialize: function() {
		...
		this.listenTo(this.controller, 'listView:deactivate', this.cleanUpAndRender, this);
		this.listenTo(this.controller, 'gridView:deactivate', this.cleanUpAndRender, this);
	},

	cleanUpAndRender: function() {
		this.undelegateEvents.bind(this);
		// console.log(this.controller);
		this.views.remove.bind(this);
		// this.unbind();
    ...
    refreshView: function() {
		...
		let sourceCollection = this.collection;
        // Clean up the view
        this.cleanupView();

        // Reset the collection
        console.log('Resetting collection');
        this.collection.reset(sourceCollection.models);
    ...
    cleanupView: function() {
        ...
        // Stop all event listeners bound using this.listenTo
        this.undelegateEvents();

        // Remove event listeners from the controller
        this.controller.off('toggle:upload:attachment', this.toggleUploader, this);
        this.controller.off('edit:selection', this.editSelection, this);

        // Clean up child views if they exist
        if (this.attachmentsWrapper) {
            console.log('Removing attachmentsWrapper');
            this.attachmentsWrapper.remove();
            this.attachmentsWrapper.off();
            this.attachmentsWrapper = null;
        }
        ...
        if (this.collection) {
            console.log('Removing collection listeners');
            this.collection.off(null, null, this);
        }

        // Remove the view's root element from the DOM
        this.views.remove();
    },
	...
	createAttachmentsWrapperView: function() {
		if (this.controller.isModeActive('listView')) {
			this.attachmentsWrapper = new wp.media.View( {
				className: 'attachments-wrapper wp-list-table widefat fixed striped media',
				tagName:   'table',
				id: "custom-media-table",
				attributes: function() {
					return {
...
Toolbar = View.extend(/** @lends wp.media.view.Toolbar.prototype */{
	...
	initialize: function() {
		...
		this.viewSwitch = new wp.media.view.PriorityList();
		this.primary.$el.addClass('media-toolbar-primary search-form');
		this.secondary.$el.addClass('media-toolbar-secondary');
		this.tertiary.$el.addClass('media-bg-overlay');
		if (this.controller.isModeActive('listView')) {
			this.primary.$el.html(`<div class="view-switch" style="height: 100%; align-content:end;margin-left: 10px;cursor: pointer;">
				<a class="view-list current" id="view-switch-list" aria-current="page"><span class="screen-reader-text">List view</span></a>
				<a class="view-grid" id="view-switch-grid"><span class="screen-reader-text">Grid view</span></a>
				</div>`);
		} else {
			this.primary.$el.html(`<div class="view-switch" style="height: 100%; align-content:end;margin-left: 10px;cursor: pointer;">
				<a class="view-list" id="view-switch-list" aria-current="page"><span class="screen-reader-text">List view</span></a>
				<a class="view-grid current" id="view-switch-grid"><span class="screen-reader-text">Grid view</span></a>
				</div>`);
		}
	...
	render: function() {
		...
		_.delay(_.bind( function() {
			var that = this;
			$('.media-frame-toolbar .view-switch').css('display', 'none');
			$('#view-switch-list').click(function() {
				var clickedItem = $(this);
				if (clickedItem.hasClass('current')) {
					return; 
				}
				var siblings = clickedItem.siblings('.current');
				if (siblings.length) {
					siblings.removeClass('current');
				}
				clickedItem.addClass('current');
				
				that.controller.activateMode('listView').deactivateMode('gridView');
			});
			$('#view-switch-grid').click(function() {
				...
				clickedItem.addClass('current');
				that.controller.activateMode('gridView').deactivateMode('listView');
			});
		}, this ), 0);
